...
app.get('/getCategory', async function (req, res) {
    ...
    var theArrayOfPolls = theCommaSeperatedString.split(',')
    ...
    for (var x of theArrayOfPolls) {
        var t = await theVoteManager.getPollInformation(x)
        var globalLive = (category === 'globalLive' && t[0].poll_status == 'LIVE')
        var globalUpcoming = (category === 'globalUpcoming' && t[0].poll_status == 'READY')
        var globalPrevious = (category === 'globalPrevious' && t[0].poll_status == 'DONE')
        var isHost = t[0].poll_host == user_id
        var myLive = globalLive && isHost
        var myDrafted = isHost && t[0].poll_status == "DRAFTED"
        var myUpcoming = isHost && globalUpcoming
        var myPrevious = isHost && globalPrevious
        var myArchived = isHost && t[0].poll_status == "ARCHIVED"
        if (myLive) console.log(`xxx ${t[0].poll_title}`)
        if (myLive || myDrafted || myUpcoming || myPrevious || myArchived) {
            theRequiredArray_ID.push(t[0].poll_id)
            theRequiredArray_Name.push(t[0].poll_title)
        } else if (globalLive || globalUpcoming || globalPrevious) {
            theRequiredArray_ID.push(t[0].poll_id)
            theRequiredArray_Name.push(t[0].poll_title)
        }
